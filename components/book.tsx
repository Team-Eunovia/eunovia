...
export function Book({
  ...
  return (
    <div
      className={cn(
        'group inline-block w-fit cursor-pointer transition-all duration-300 [perspective:1200px] h-fit h-60',
      )}
      style={
        {
          '--book-color': color,
            ...
            >
              ...
              {(title || author) && (
                <div className='p-4 text-center'>
                  {title && (
                    <h3 className={cn('mb-2 text-lg font-bold')} style={{ color: textColor }}>
                      {title}
                    </h3>
                  )}
                  {author && (
              ...
              {/* Illustration */}
              {illustration && (
                <div className='flex flex-1 items-center justify-center p-4'>{illustration}</div>
              )}
